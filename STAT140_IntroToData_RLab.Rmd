...
*This is an R Markdown version of the Introduction to R and RStudio Lab available through OpenIntro. This lab was adapted for OpenIntro by Andrew Bray and Mine Cetinkaya-Rundel from a lab written by Mark Hansen of UCLA Statistics. The lab is available at* \href{"http://htmlpreview.github.io/?https://github.com/andrewpbray/oiLabs-base-R/blob/master/intro_to_r/intro_to_r.html"}{OpenIntro: Introduction to R and R Studio}. 

Last week we did a few R exercises in class using Dr. Arbuthnot's baptism records. We are going to revisit that data today to review some of the basic R concepts we learned. Then, we will compare these data to present day birth records in the United States.

To get you started, run the following command in the R chunk. This will read in the data from the OpenIntro website.
...
What command would you use to extract just the counts of girls baptized? Try it! Be sure to insert an R chunk below this to carry out this exercise, and name it appropriately.
...
```{r simple plot year girls}
plot(x=arbuthnot$year, y=arbuthnot$girls)
```

Note that we could accomplish the same thing using the ggplot() function from the ggplot2 package. In the chunk below, load the ggplot2 package and then make the same plot using ggplot().
...
```{r simple line plot year girls}
plot(x = arbuthnot$year, y = arbuthnot$girls, type = "l")
```

Note that we could accomplish the same thing using the ggplot() function from the ggplot2 package. In the chunk below, load the ggplot2 package and then make the same plot using ggplot().

```{r ggplot line year girls}
...
Is there an apparent trend in the number of girls baptized over the years? How would you describe it? Be sure to insert an R chunk below this to carry out this exercise, and name it appropriately.

Now, suppose we want to plot the total number of baptisms. To compute this, we could use the fact that R is really just a big calculator. We can type in mathematical expressions like
```{r total baptisms arbuthnot}
5218+4683
```
to see the total number of baptisms in the year 1629 is 9901. We could repeat this once for each year, but there is a faster way. If we add the vector for baptisms for boys and girls, R will compute all sums simultaneously.

```{r add all elementwise arbuthnot}
arbuthnot$boys + arbuthnot$girls
```

What you will see are 82 numbers (in that packed display, because we aren’t looking at a data frame here), each one representing the sum we’re after. Take a look at a few of them and verify that they are right. Now, store the sum that you did in the previous R chunk in a new column of arbuthnot that you create called total. Do this in a new R chunk that you insert below and name appropriately.

Therefore, we can make a plot of the total number of baptisms per year with the command
```{r simple plot year total baptisms}
...
Note that we could accomplish the same thing using the ggplot() function from the ggplot2 package. In the chunk below, load the ggplot2 package and then make the same plot using ggplot(). You will need to use the total variable from the arbuthnot data, rather than the sum of boys and girls for ggplot().

```{r ggplot line year total baptisms}
...
Now, make a plot of the proportion of boys over time. What do you see? Tip: If you use the up and down arrow keys, you can scroll through your previous commands, your so-called command history. You can also access it by clicking on the history tab in the upper right panel. This will save you a lot of typing in the future. Be sure to insert an R chunk below this to carry out this exercise, and name it appropriately.
...
```{r boys outnumber girls arbuthnot}
arbuthnot$boys > arbuthnot$girls
```
...
In what year did we see the most total number of births in the U.S.? You can refer to the help files or the \href{"http://cran.r-project.org/doc/contrib/Short-refcard.pdf}{R reference card} to find helpful commands.

These data come from a report by the \href{"http://www.cdc.gov/nchs/data/nvsr/nvsr53/nvsr53_20.pdf"}{Centers for Disease Control}. Check it out if you would like to read more about an analysis of sex ratios at birth in the United States.
